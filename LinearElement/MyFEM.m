classdef MyFEM
    ...
    properties(Dependent)
        ...
        A; % Constraint matrix
        b; % Constraint vector
%         Aaug; % Augmented system matrix
%         baug; % Augmented system vector
%         x; % Contains node displacements and vincular reactions [u; λ]
...
%         fi; % Forces on nodes
    ...
    methods
        ...
        function K = get.K(obj)
            ...
            K = sparse(6 * nBeams, 6 * nBeams);
            for i = 1:nBeams
                idx = 6 * (i - 1) + 1;
                K(idx:idx+5,idx:idx+5) = obj.beams{i}.K;
            end
        end

        function f = get.f(obj)
            flen = numel(obj.beams) * 2 * 3;
            f = sparse(flen,1);
            % Forces contribution
            ...
            for i = 1:numel(obj.beams)
                idx = 6 * (i - 1) + 1;
                f(idx:idx+5) = f(idx:idx+5) + obj.beams{i}.w;
            end
        end
...
%         function Aaug = get.Aaug(obj)
%             Arows = size(obj.A,1);
%             Aaug = [ obj.K              obj.A' ;
%                      obj.A sparse(Arows,Arows) ];
%         end
% 
%         function baug = get.baug(obj)
%             baug = [ obj.f; obj.b ];
%         end
% 
...
%         function [Fx,Fy,Mz] = getNodeForce(obj,beamIdx,nodeIdx)
%             idx = 6 * (beamIdx - 1) + 3 * (nodeIdx - 1) + 1;
% 
%             Fx = full(obj.fi(idx));
%             Fy = full(obj.fi(idx + 1));
%             Mz = full(obj.fi(idx + 2)) * 1e-3;
%         end
    end
end